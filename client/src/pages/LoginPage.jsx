...
const LoginPage = () => {
  ...
  const onSubmitHandler = async (event)=>{
    ...
    const isSuccess = await login(currState==="Sign up" ? 'signup' : 'login', {fullName, email, password, bio});
    if (isSuccess) {
      navigate('/');
    }
  }
  ...
  return (
    <div className="min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl">
      ...
      <h2 className='font-medium text-2xl flex justify-between items-center'>
        ...
        {
          currState === "Sign up" && isDataSubmitted && (
            <textarea onChange={(e)=>setBio(e.target.value)} value={bio} rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-ingigo-500' placeholder='provide a short bio...' required></textarea>
          )
        }
